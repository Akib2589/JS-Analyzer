...
DEFAULT_MODEL = "gemini-2.5-pro" # SWITCHED to Pro for deeper, more complex code analysis
...
def call_gemini_api(messages: List[Dict], max_tokens: int, model: str = DEFAULT_MODEL) -> str:
    ...
    payload = {
        ...
        "generationConfig": {
            ...
            "temperature": 0.3, # Slightly increased from 0.1 to help model process complex code
        }
    }
    ...
    max_retries = 5 # Increased from 3 to 5 for better resilience against transient errors
    delay = 35 # Increased from 5 seconds to 35 seconds to safely wait for the 30s quota reset

    for attempt in range(max_retries):
        try:
            # Increased timeout from 120s to 180s (3 minutes) for large chunks
            ...
            # ** FIX: Use defensive logic to safely extract text and prevent 'list index out of range' **
            ...
            # If we reach here, either no candidates were returned, or the structure was incomplete/unexpected
            ...
            # Handle cases where the model returns candidates but no text part, or no candidates at all
            return "Error: Gemini API returned no valid content candidates or the response structure was unexpected."

        except requests.exceptions.HTTPError as e:
            # Handle common HTTP errors (e.g., 400 Bad Request, 429 Rate Limit, 503 Server Unavailable)
            ...
            # Catch unexpected exceptions (including IndexError if it somehow still occurs)
...
def chunk_file_and_analyze(filename: str, code_content: str, model: str) -> str:
    ...
    for i, chunk in enumerate(chunks):
        ...
        "Analyze this specific chunk for **all possible security issues** including but not limited to: "
        "hardcoded secrets, API keys, endpoints, URLs, request parameters, headers, XSS sources and sinks, DOM sinks "
        "(innerHTML, outerHTML, insertAdjacentHTML, document.write), template injections, eval/new Function usage, "
        "insecure storage (localStorage, sessionStorage, cookies), crypto misuse, CORS misconfigurations, unsafe network calls, "
        "open redirects, risky third-party libraries, debug logs, and suspicious comments. "
        "Output ONLY a JSON array of findings. For each finding include: type, severity (CRITICAL/HIGH/MEDIUM/LOW), "
        "confidence (HIGH/MEDIUM/LOW), chunk_lines (relative to this chunk), file_char_start/end, summary, evidence (short snippet), "
        "exploit_concept (one-line, conceptual), recommendation (concise fix), and optional metadata. "
        ...
        # Send the system and user messages to the Gemini API, passing the model
        try:
            analysis_text = call_gemini_api([system_msg, user_msg], MAX_CHUNK_ANALYSIS_TOKENS, model)
    ...
    try:
        final_report = call_gemini_api([summary_system_msg, summary_user_msg], MAX_SUMMARY_TOKENS, model)
        return final_report
    except Exception:
...
def main():
    ...
    # The model is now sourced directly from the command-line arguments (args.model).
    ...
    # Use args.model directly for logging
    ...
    try:
        # Pass the desired model from the arguments to the analysis function
        final_analysis = chunk_file_and_analyze(input_path.name, code_content, args.model)
    except Exception:
